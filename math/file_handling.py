...
def merge_dates_and_towns_into_csv(dates_filename: str, towns_filename: str, csv_output_filename: str) -> None:
    ...
    with open(dates_filename, 'r') as dates_file:
        dates_data = {line.split(':')[0]: line.split(':')[1].strip() for line in dates_file}

    with open(towns_filename, 'r') as towns_file:
        towns_data = {line.split(':')[0]: line.split(':')[1].strip() for line in towns_file}

    with open(dates_filename, 'r') as dates_file:
        names_order = [line.split(':')[0].strip() for line in dates_file]

    for name in towns_data.keys():
